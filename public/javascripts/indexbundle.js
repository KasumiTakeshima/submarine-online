...
function drawRadar(ctxRader) {
  //中心座標と半径を計算したら、xyrの変数にそれぞれ代入している。
  var x = gameObj.raderCanvasWidth / 2;
  var y = gameObj.raderCanvasHeight / 2;
  var r = gameObj.raderCanvasWidth * 1.5 / 2; // 対角線の長さの半分

  ctxRader.save(); // 現在のキャンバスの状態を保存

  ctxRader.beginPath(); // 新しい描画を始める
  ctxRader.translate(x, y); //座標を x と y に設定し、画面の中央が座標になるようにする。
  ctxRader.rotate(getRadian(gameObj.deg));

  ctxRader.fillStyle = 'rgba(0, 220, 0, 0.5)'; //半透明の緑

  //レーダーの形
  ctxRader.arc(0, 0, r, getRadian(0), getRadian(-30), true); //扇の弧の部分を 原点(0, 0) を中心にして半径 r で30度だけ描画
  ctxRader.lineTo(0, 0);

  ctxRader.fill(); //エリアを塗りつぶしている

  ctxRader.restore(); // 元の設定を取得
  gameObj.deg = (gameObj.deg + 5) % 360; //角度を5度ずつ変えていくことで、回転していってくれる。10にすると速く、2にすると遅くなる。
}

function drawSubmarine(ctxRader) {
  ctxRader.save(); //canvas の状態を保存
  ctxRader.translate(gameObj.raderCanvasWidth / 2, gameObj.raderCanvasHeight / 2); //座標を canvas の中心に設定

  ctxRader.drawImage(gameObj.submarineImage, -(gameObj.submarineImage.width / 2), -(gameObj.submarineImage.height / 2) //ど真ん中に表示するため。これをかかないと、左角が真ん中に設定されるので画像が少し右下にずれる。
  );
  ctxRader.restore();
}
